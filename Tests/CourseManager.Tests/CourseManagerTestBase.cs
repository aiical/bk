...
{
    public abstract class CourseManagerTestBase : AbpIntegratedTestBase<CourseManagerTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected CourseManagerTestBase()
        {
        ...
        protected void UsingDbContext(Action<CourseManagerDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CourseManagerDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CourseManagerDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CourseManagerDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CourseManagerDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CourseManagerDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CourseManagerDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CourseManagerDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CourseManagerDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CourseManagerDbContext, Task<T>> func)
        {
            T result;
