...
namespace CourseManager.Common
{
	...
	{
		#region 各种验证 正则（邮件，手机号，固话，IP,日期，邮政编码等）
		//邮件正则表达式
		...
		public static bool InIP(string sourceIP, string targetIP)
		{
			...
			string[] sourceIPBlockList = Helper.SplitString(sourceIP, @".");
			string[] targetIPBlockList = Helper.SplitString(targetIP, @".");
		...
		{
			string[] targetIPList = Helper.SplitString(targetIPStr, "\n");
		...
		public static bool BetweenPeriod(string[] periodList, out string liePeriod)
		{
			if (periodList != null && periodList.Length > 0)
			{
				...
				foreach (string period in periodList)
				{
					...
					startTime = Helper.StringToDateTime(period.Substring(0, index));
					endTime = Helper.StringToDateTime(period.Substring(index + 1));
		...
		{
			string[] periodList = Helper.SplitString(periodStr, "\n");
		...
		{
			return IsNumericArray(Helper.SplitString(numericRuleStr, splitChar));
		}
