...
!function ($) {
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.showColumns) {
            html = [];
            html.push('<div class="columns pull-left keep-open">',
            ...
            $keepOpen.find('input').off('click').on('click', function () {
                ...
                var _isC = $this.prop('checked');
                var thatClomn = that.options.columns[$this.val()];
                thatClomn.visible = _isC;
                //更新列显示缓存 
                if (thatClomn.field && that.options.extend && that.options.extend.cacheDisplay) {
                    if (window.localStorage) {
                        var _key = 'bootstrapTableColumnsDisplayData' + (that.options.extend.pageLabel || window.location.pathname);
                        var storage = window.localStorage;
                        var _cData = storage.getItem(_key);
                        var isCache = false;
                        if (_cData) {
                            isCache = true;
                            var cD = _cData.split(",");
                            if (_isC) {
                                cD.push(thatClomn.field);
                                var newArray = [];
                                var provisional = {};
                                for (var i = 0, item; (item = cD[i]) != null; i++) {
                                    if (!provisional[item]) {
                                        newArray.push(item);
                                ...
                                var i = cD.length;
                                while (i--) {
                                    if (cD[i] === thatClomn.field) {
                                        cD.splice(i, 1);
                                        break;
                                    }
                                }
                            }
                            storage.setItem(_key, cD.join(","));
                        }
                    }
                }
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        if (window.localStorage) { //cache pageSize
            if (this.options && this.options.extend && this.options.extend.cacheDisplay === true) {
                var key = 'pageSizeKey' + (this.options.extend.pageLabel || window.location.pathname);
                var storage = window.localStorage;
                var cachePageSizeData = storage.getItem(key);
                //console.log('init-cacheSize:' + cachePageSizeData);
                if (cachePageSizeData != 'null' && cachePageSizeData != null) {
                    this.options.pageSize = cachePageSizeData;
                    $('.page-list>.dropdown-menu>li').each(function () {
                        var $href = $(this).find('a');
                        $href.removeClass('active');
                        if ($href.html == this.options.pageSize) {
                            $href.addClass('active');
                        }
                    });
                } else {
                    window.localStorage.setItem(key, this.options.pageSize);
                }
            }
        }
        var that = this,
        ...
        var pageNumber = [
		    ...
		    ' <span class="caret"></span>',
		    '</button>',
		    '<ul class="dropdown-menu custom-page-menu" role="menu">'],
    ...
    BootstrapTable.prototype.onPageListChange = function (event) {
        ...
        if (window.localStorage) { //cache Size
            if (this.options && this.options.extend && this.options.extend.cacheDisplay === true) {
                var key = 'pageSizeKey' + (this.options.extend.pageLabel || window.location.pathname);
                window.localStorage.setItem(key, this.options.pageSize);
            }
    ...
    BootstrapTable.prototype.initBody = function () {
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                if (typeof that.options.columns[j] != 'undefined') {
    ...
    $.fn.bootstrapTable = function (option, _relatedTarget) {
        if (window.localStorage) {//缓存用户设置的显示列
            if (option && option.columns && option.extend && option.extend.cacheDisplay === true) {
                var _key = 'bootstrapTableColumnsDisplayData' + (option.extend.pageLabel || window.location.pathname);
                var storage = window.localStorage;
                var _cData = storage.getItem(_key);
                var isCache = false;
                if (_cData) {
                    ...
                    if (cD && cD.length > 0) {
                        isCache = true;
                        $.each(option.columns, function (q, o) {
                            var show = false;
                ...
                if (!isCache) {
                    var _showColumns = [];
                    if (typeof option.columns == "object") {
                        $.each(option.columns, function () {
                            if (this.visible === true || this.visible === undefined) {
                                _showColumns.push(this.field);
                            }
                        });
                    }
                    window.localStorage.setItem(_key, _showColumns.join(","));
                }
            }
        }
        var allowedMethods = [
