...
!function ($) {
	...
	BootstrapTable.prototype.initTable = function () {
		...
		this.options.columns = $.extend([], columns, this.options.columns); //将
	...
	BootstrapTable.prototype.initToolbar = function () {
		...
		if (this.options.showColumns) { //设定了要切换列显示
			...
			html.push('</ul>',
			    '</div>');
			console.log("切换列显示的Html添加到容器：" + html);
		}
		html.push('</div>');
		if (html.length > 2) {
	...
	$.fn.bootstrapTable.Constructor = BootstrapTable;  //构造bootstrapTable
	$.fn.bootstrapTable.defaults = BootstrapTable.DEFAULTS;  //默认 属性 方法 事件等
	$.fn.bootstrapTable.columnDefaults = BootstrapTable.COLUMN_DEFAULTS; //列默认属性值
	...
	$(function () {
		$('[data-toggle="table"]').bootstrapTable(); //调用入口  jquery扩展的实例方法$.fn.bootstrapTable
	});
